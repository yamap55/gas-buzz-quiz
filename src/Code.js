/**
 * 早押し4択クイズ アプリ本体。
 * 進行状態は ScriptProperties を真実源とし、スプレッドシートはデータの置き場として使う。
 * 素直な実装に直すと壊れる箇所については README の「実装上の注意点」を参照。
 */
...
/**
 * 参加者画面のポーリングから呼ばれる。
 * スプレッドシートには触らない。読み取りは1回1〜2秒かかり、参加者の人数だけ
 * 数秒ごとに走るため、Apps Script の同時実行上限（ユーザーあたり30）に近づく。
 ...
 * 回答を記録する。
 *
 * 順位に使うのは payload.elapsedMs（クライアントが「押した時刻 - startAt」で
 * 算出した値）で、サーバー側で測る serverElapsedMs は参考値として残すだけ。
 * サーバーの受信時刻にはポーリング間隔と通信遅延が乗るため、順位に使うと
 * 回線の速い端末が有利になる。
 *
 * 二重送信はクライアント側で防ぐ想定のため、ここでは弾かない。
 * すり抜けた場合は集計にそのまま乗り、重複した分だけ正解数とタイムが増える。
 */
...
/**
 * 最終結果シートを組み立てる。順位の決め方は README を参照。
 *
 * 集計対象の名前は参加者シートと回答シートの両方から集める。参加者シートだけを
 * 見ると、名前が一致しない回答が黙って0正解に沈み、気づけないため。
 *
 * 集計はG列より右の作業用セルに一度出してから並べ替える。検索条件に計算結果の
 * 配列を渡すと展開されないため、条件には実際のセル範囲しか渡せない。
 * 合計タイムは SUMIFS が展開されず0を返すため、回答1行ごとの有効タイムを
 * K列に出したうえで、条件が1つだけの SUMIF で合計している。
 */
